...
#define BootDeviceSataId   0x0
#define BootDeviceGe5Id    0x1
#define BootDeviceGe6Id    0x2
#define BootDeviceGe7Id    0x3
#define MaxBootDeviceNum   0x4
UINT8 gBootSataDeviceNum=0;
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    if ((NULL == DefaultConsolePaths) || ( L'\0' == DefaultConsolePaths[0])) {
      *DevicePaths = NULL;
      return EFI_SUCCESS;
    }
...
EFI_STATUS StringFind(const CHAR16 *pSrc, const CHAR16 *pDst)  
{  
    int i, j;  
    for (i=0; pSrc[i]!='\0'; i++)  
    {  
...
extern VOID DisplayBootOptions (
  ...
  )
{
  ...
  if (Timeout != 0xFFFF) {
    if (Timeout > 0) {
      ...
      Status = gBS->CreateEvent (EVT_TIMER, 0, NULL, NULL, &WaitList[0]);
      Status = gBS->SetTimer (WaitList[0], TimerPeriodic, EFI_SET_TIMER_TO_SECOND);
  ...
  )
{
  ...
  //?¨¬1??¨²¡ä?¨¦?¡À¡§¨¢¨´¨º¡À1?¡À¨¹¡¤?¡ã?¡ê?¦Ì¡À¨¦¨º??2G?¨²¡ä?¨º¡À¡ê??¨²¡ä?2?1?¨®?¡ê??¨ª?¨¢??HOB?D
  //memory reserved ¨¤¨¤D¨ª¨¬¨¢¨¦y?asystem memory ¨¤¨¤D¨ª
  ...
  Status = DefineDefaultBootEntries ();
  ...
  Status = StartDefaultBootOnTimeout ();
