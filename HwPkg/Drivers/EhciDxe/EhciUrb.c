...
{
   return (UINT32) ((EHC_LOW_32BIT (Addr) & 0xFFFFFFE0) | (Type) | ((Term) ? 1 : 0));
}
  ...
  )
{
  ...
  QtdHw             = &Qtd->QtdHw;
  QtdHw->NextQtd    = QTD_LINK ((EFI_PHYSICAL_ADDRESS)(UINTN)NULL, TRUE);
  QtdHw->AltNext    = QTD_LINK ((EFI_PHYSICAL_ADDRESS)(UINTN)NULL, TRUE);
  ...
  )
{
  ...
  QhHw                = &Qh->QhHw;
  QhHw->HorizonLink   = QH_LINK ((EFI_PHYSICAL_ADDRESS)(UINTN)NULL, 0, TRUE);
  ...
  )
{
  if (Urb->RequestPhy != NULL) {
    (VOID)MemUnmap(Urb->RequestMap);
  }

  if (Urb->DataMap != NULL) {
    (VOID)MemUnmap(Urb->DataMap);
  }
  ...
  )
{
  ...
  Ep        = &Urb->Ep;
  StatusQtd = NULL;
  AlterNext = QTD_LINK ((EFI_PHYSICAL_ADDRESS)(UINTN)NULL, TRUE);
  ...
  )
{
  ...
  if (Request != NULL) {
    ...
    Status  = MemMap (PciIo, MapOp, Request, &Len, &PhyAddr, &Map); 
  ...
  if (Data != NULL) {
    ...
    Status  = MemMap (PciIo, MapOp, Data, &Len, &PhyAddr, &Map);
