  ...
  )
{
  ...
  if(Pool->Check4G) {
    ...
    BufHost = UncachedAllocatePagesBelow4G(Pages);
  }
  ...
  Status  = MemMap (PciIo, EfiPciIoOperationBusMasterCommonBuffer, BufHost, &Bytes, &MappedAddr, &Mapping);
  if (EFI_ERROR (Status) || (Bytes != EFI_PAGES_TO_SIZE (Pages))) {
    goto FREE_BUFFER;
  }
  //uniBIOS_y00216284_037_end 2015-4-28 <<<
...
FREE_BUFFER:
  UncachedFreePages (BufHost, Pages);
  ...
  )
{
  ...
  //
  (VOID)MemUnmap(Block->Mapping);
  UncachedFreePages (Block->BufHost, EFI_SIZE_TO_PAGES (Block->BufLen));
  ...
  )
{
  ...
  if(Head == NULL || BlockToUnlink == NULL) {
    return;
  }
