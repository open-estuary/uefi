  ...
  )
{
  ...
  if(EHC_BIT_IS_SET (Ehc->HcCapParams, HCCP_64BIT)) {
    Buf = UncachedAllocatePages(Pages); 
  } else {
    Buf = UncachedAllocatePagesBelow4G(Pages);
  } 

  if(NULL == Buf)  { 
    DEBUG((EFI_D_ERROR,"[ERROR]:UncachedAllocatePages fail!\n"));
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  )
{
  ...
  if (Ehc->MemPool != NULL) {
    (VOID)UsbHcFreeMemPool (Ehc->MemPool);
    Ehc->MemPool = NULL;
  }
  ...
  if (Ehc->PeriodFrame != NULL) {
    ...
    (VOID)MemUnmap(Ehc->PeriodFrameMap);
	UncachedFreePages (Ehc->PeriodFrame, EFI_SIZE_TO_PAGES (EFI_PAGE_SIZE));
  ...
  )
{
  ...
  if((Ehc == NULL) || (Urb == NULL) || (Urb->Qh == NULL)) {
    return FALSE;
  }
  ...
  )
{
  ...
  EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Ehc->AsyncIntTransfers) {
    ...
    if ((Urb->Ep.DevAddr == DevAddr) && (Urb->Ep.EpAddr == EpNum) &&
      ...
      //
      (VOID)EhcCheckUrbResult (Ehc, Urb);
  ...
  )
{
  ...
  Status  = MemMap (PciIo, MapOp, Urb->Data, &Len, &PhyAddr, &Map);
  if (EFI_ERROR (Status) || (Len != Urb->DataLen)) {
    goto ON_ERROR;
  }
